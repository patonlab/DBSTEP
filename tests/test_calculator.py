...
quadrant_points_2d = [[0.5, 1.0], [0.5, -1.0], [-0.5, 1.0], [-0.5, -1]]


@pytest.fixture(params=quadrant_points_2d, ids=[str(point) for point in quadrant_points_2d])
def quadrant_point(request):
	"""parametrized with four quadrants of a 2D circle."""
	return request.param


axis_combinations = [("z", "x", "y"), ("x", "z", "y"), ("y", "x", "z"), ("x", "y", "z"), ("z", "y", "x"), ("y", "z", "x")]


@pytest.fixture(params=axis_combinations, ids=[str(comb) for comb in axis_combinations])
...
def test_angle_between_axis(axis_combination, vector_to_rotate):
	...
	error_message = f"failed to rotate {vector_to_rotate} using from: {axis_from_index}, to: {axis_to_index}\n"
